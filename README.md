...
⚙️ Methodology
Data Cleaning — Dropped rows with missing target values (Life expectancy), removed the Country column, and filled remaining missing feature values with the column mean
Feature Encoding — Converted the categorical Status column (Developed/Developing) into numeric form
Scaling & Split — Split data 80/20 into train/test sets, then standardized features (scaler fit on training data only, to avoid data leakage)
Model Training — Trained three regression models: Linear Regression, Decision Tree, and Random Forest
Evaluation — Compared models using MAE, MSE, RMSE, and R² score on the held-out test set
📈 Results
